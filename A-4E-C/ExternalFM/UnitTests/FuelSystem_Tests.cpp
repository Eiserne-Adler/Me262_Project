...
namespace UnitTests
{
	...
	public:
		...
		TEST_METHOD( AddFuel1 )
		{
			...
			Scooter::Engine1 engine1b(state);

			Scooter::FuelSystem2 system(engine, engine1b, state );
		...
		TEST_METHOD( Update1 )
		{
			...
			Scooter::Engine1 engine1b(state);
			engine1b.m_hpOmega = 1000;
			engine1b.m_lpOmega = 1000;

			Scooter::FuelSystem2 system( engine, engine1b, state );
			system.setInternal( 500 );
			engine.m_correctedFuelFlow = 1.0;
			engine.m_fuelFlow = 1.0;
			engine1b.m_correctedFuelFlow = 1.0;
			engine1b.m_fuelFlow = 1.0;
			//dt = 1.0 s dm = 1.0 kg
			system.update( 1.0 );
			system.update( 1.0 );
			system.update( 1.0 );
