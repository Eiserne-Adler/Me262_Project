...
static Scooter::Engine1* s_engine = NULL; //
...
void ed_fm_simulate(double dt)
{
	...
	if ( s_interface->getAvionicsAlive() )
	{
		...
		s_engine->setThrottle1(s_interface->getEngineThrottlePosition1()); //
		s_engine->setBleedAir1(s_interface->getBleedAir1() > 0.1); //
		s_engine->setIgnitors1(s_interface->getIgnition1() > 0.1); //
